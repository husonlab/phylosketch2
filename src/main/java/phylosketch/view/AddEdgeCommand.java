...
public class AddEdgeCommand extends UndoableRedoableCommand {
		...
		redo = () -> {
			try {
				...
				if (sourceNodeId != -1 && network.findNodeById(sourceNodeId) != null) {
					...
					drawPane.getEdgeSelection().getSelectedItems().remove(sourceEdge);
				...
				if (targetNodeId != -1 && network.findNodeById(targetNodeId) != null) {
					...
					drawPane.getEdgeSelection().getSelectedItems().remove(targetEdge);
		...
		undo = () -> {
			try {
				// remove the edge
				{
					...
					drawPane.getEdgeSelection().getSelectedItems().remove(e);
				...
				if (sourceNodeCreated) {
					...
					drawPane.getNodeSelection().getSelectedItems().remove(v);
					...
					drawPane.getEdgeSelection().getSelectedItems().remove(e1);
					...
					drawPane.getEdgeSelection().getSelectedItems().remove(e2);
	...
	private Triplet<Node, Edge, Edge> splitExistingEdge(Point2D point, Edge edge, Path path, DrawPane drawPane) {
		...
		if (index != -1) {
			...
			var path1 = new Path();
			path1.getStyleClass().add("graph-edge");
			Node start1 = null;
