...
import phylosketch.window.SelectionManager;
...
public class DrawPane extends Pane {
	...
	public DrawPane() {
		...
		graphFX.lastUpdateProperty().addListener(e -> {
			...
			for (var v : graph.nodes()) {
				if(v.getData() instanceof Shape shape) {
					...
					SelectionManager.setupNodeSelection(v, shape, nodeSelection, edgeSelection);
				}
			}

			for (var edge : graph.edges()) {
				if(edge.getData() instanceof Path ePath)
					SelectionManager.setupEdgeSelection(edge, ePath, nodeSelection, edgeSelection);
			}
		});
		...
		SelectionManager.setupPaneSelection(this, nodeSelection, edgeSelection);
	}
	...
	public String toBracketString(boolean showWeights) {
		var root = graph.nodeStream().filter(v -> v.getInDegree() == 0).findAny();
		if (root.isPresent()) {
			graph.setRoot(root.get());
			graph.edgeStream().forEach(f -> {
				var reticulate = (f.getTarget().getInDegree() > 1);
				graph.setWeight(f, reticulate ? 0 : 1);
				graph.setReticulate(f, reticulate);
			});
			return graph.toBracketString(showWeights) + ";";
		} else return "?";
	}
